...
# Project Progress
| DDL 	| 任务     	| 龙良曲 	| 陈浩 	| 李国豪 	|朱雷|
|------	|----------	|--------	|------	|--------	|--------	|
| 6-23 	| 任务分配 	|  第二部分（除TF基础），AE,GAN, Attention	|   第一部分，TF基础,Keras  	|  RL      	| CNN RNN|
| 7-28     	|   约完成20%       	|  回归问题，分类问题      	|  TF基础    	|    RL(30%)    	| -|
| 8-28 	|   约完成40%         	|    第二部分    	|   Keras    	|     RL全部   	|	   CNN全部     	|
|      	|    约完成60%        	|        	|      	|        	|       	|
|      	|   约完成80%         	|        	|      	|        	|       	|
|  12-31    	|     100%     	|   合并成书并校验     	| 校验     	| 校验       	|      校验  	|
...
# 编写建议
建议大家从讲故事、引入问题并如何一步步解决问题的角度，把相关的知识点串起来讲，避免给定义，灌输概念，教条式写法；并配以生动图片和精简的代码段，帮助理解。
可以先编写内容，细节的格式可以后续调整。

我们会发布2个版本：
- 开源书版本，支持彩色图片，高亮代码，彩色曲线，超链接等交互性元素
- 印刷版本，主要以灰度素材为主
